...
class PipelineManager {
  public:
    ...
    AsyncDatabaseAccessor *getAsyncDatabaseAccessor() {
        ...
        if (!threads_opened_) {
            throw DBException(
                "Cannot access the AsyncDatabaseAccessor before calling openPipelines()");
        }
    ...
    std::unique_ptr<ScopedRunnableDisabler> scopedDisableAll(bool disable_threads_too = true) {
        ...
        if (disable_threads_too) {
            disabler.reset(
                new ScopedRunnableDisabler(this, disabler_runnables_, disabler_threads_));
        } else {
  ...
  private:
    ...
    void getDisablerRunnables_() {
        ...
        for (auto &thread : polling_threads_) {
            ...
            disabler_runnables_.insert(disabler_runnables_.end(), runnables.begin(),
                                       runnables.end());
        }
    ...
    void onDisablerDestruction_() {
        if (!disabler_active_) {
            throw DBException("Internal error: no disabler active in onDisablerDestruction_()");
        }
...
/// Defined here so we can avoid circular includes
template <typename KeyType, typename SnoopedType>
bool PipelineSnooper<KeyType, SnoopedType>::snoopAllStages(const KeyType &key,
