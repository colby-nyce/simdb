...
class ThreadMerger {
  public:
    ...
    void addAppForMerging(const App *app) {
        ...
        if (merge_all_apps_) {
            throw DBException(
                "Can either call addAppForMerging() or mergeAllAppThreads(), not both");
        }
    ...
    void performMerge(std::vector<std::unique_ptr<PollingThread>> &polling_threads) {
        ...
        for (const auto &[runnable, threads] : threads_by_runnable) {
            if (threads.size() != 1) {
                throw DBException(
                    "Internal error - assigned pipeline stage to more than one thread");
            }
        }
  ...
  private:
    void
    createThreadsWithoutMerging_(std::vector<std::unique_ptr<PollingThread>> &polling_threads) {
    ...
    void createThreadsAndMergeAll_(std::vector<std::unique_ptr<PollingThread>> &polling_threads) {
        ...
        if (intervals.size() != 1) {
            throw DBException(
                "In order to merge threads, all must agree on their polling interval.");
        }
    ...
    void createThreadsAndMergeSpecificApps_(
        std::vector<std::unique_ptr<PollingThread>> &polling_threads) {
        ...
        for (const auto &[app, app_polling_threads] : polling_threads_by_app) {
            ...
            if (merge) {
                max_polling_threads_merged =
                    std::max(max_polling_threads_merged, app_polling_threads.size());
            }
        }
        ...
        for (auto app : apps_to_merge_) {
            ...
            for (const auto &[name, stage] : app_pipeline->getOrderedStages()) {
                ...
                std::sort(polling_threads.begin(), polling_threads.end(),
                          [](const std::unique_ptr<PollingThread> &thread1,
                             const std::unique_ptr<PollingThread> &thread2) {
