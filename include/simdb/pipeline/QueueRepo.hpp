...
template <typename T> class InputQueuePlaceholder : public QueuePlaceholder {
  public:
    ...
    InputQueuePlaceholder(ConcurrentQueue<T> *&queue) : queue_(queue) {
        if (queue_) {
            throw DBException(
                "Input queue placeholder cannot be initialized with non-null queue pointer.");
        }
    }
...
template <typename T> class OutputQueuePlaceholder : public QueuePlaceholder {
  public:
    ...
    OutputQueuePlaceholder(ConcurrentQueue<T> *&queue) : queue_(queue) {
        if (queue_) {
            throw DBException(
                "Output queue placeholder cannot be initialized with non-null queue pointer.");
        }
    }
...
class StageQueueRepo {
  public:
    template <typename T>
    void addInPortPlaceholder(const std::string &port_name, ConcurrentQueue<T> *&queue) {
        ...
        if (placeholder) {
            throw DBException("Input port placeholder '" + port_name +
                              "' already exists in QueueRepo");
        }
        placeholder = std::make_unique<InputQueuePlaceholder<T>>(queue);
    }

    template <typename T>
    void addOutPortPlaceholder(const std::string &port_name, ConcurrentQueue<T> *&queue) {
        if (!stage_name_.empty()) {
            throw DBException("You may only add in/out ports in Stage subclass constructors");
        }
        ...
        if (placeholder) {
            throw DBException("Output port placeholder '" + port_name +
                              "' already exists in QueueRepo");
        }
  ...
  public:
    void merge(StageQueueRepo &other) {
        if (state_ != RepoState::ACCEPTING_STAGES) {
            throw DBException(
                "Cannot merge StageQueueRepo; PipelineQueueRepo not accepting stages.");
        }

        for (auto &[key, placeholder] : other.input_placeholders_) {
            if (input_placeholders_.find(key) != input_placeholders_.end()) {
                throw DBException("Input port placeholder '" + key +
                                  "' already exists in QueueRepo");
            }
        ...
        for (auto &[key, placeholder] : other.output_placeholders_) {
            if (output_placeholders_.find(key) != output_placeholders_.end()) {
                throw DBException("Output port placeholder '" + key +
                                  "' already exists in QueueRepo");
            }
    ...
    void finalizeBindings() {
        if (state_ != RepoState::ACCEPTING_BINDINGS) {
            throw DBException(
                "Cannot finalize bindings; PipelineQueueRepo not accepting bindings.");
        }
        ...
        for (const auto &[out_port, in_port] : port_bindings_) {
            ...
            if (out_it == output_placeholders_.end()) {
                throw DBException("Output port placeholder '" + out_port +
                                  "' not found in QueueRepo");
            }
            auto in_it = input_placeholders_.find(in_port);
            if (in_it == input_placeholders_.end()) {
                throw DBException("Input port placeholder '" + in_port +
                                  "' not found in QueueRepo");
            }
    ...
    template <typename T> ConcurrentQueue<T> *getInPortQueue(const std::string &port_full_name) {
        ...
        if (it == input_placeholders_.end()) {
            throw DBException("Input port placeholder '" + port_full_name +
                              "' not found in PipelineQueueRepo");
        }
    ...
    template <typename T> ConcurrentQueue<T> *getOutPortQueue(const std::string &port_full_name) {
        ...
        if (it == output_placeholders_.end()) {
            throw DBException("Output port placeholder '" + port_full_name +
                              "' not found in PipelineQueueRepo");
        }
