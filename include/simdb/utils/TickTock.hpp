        ...
        for (auto avg_time : sorted_avg_times)
        {
            for (const auto& [method_name, method_results] : results_)
            {
                ...
                std::cout << "called " << method_results.first << " time" << (method_results.first > 1 ? "s" : "")
                          << " ";
                std::cout << ", " << std::setprecision(6) << method_results.second / method_results.first
                          << " avg S+C seconds\n";
            }
        }
    }
...
private:
    ...
    using AllResults = std::unordered_map<std::string,
                                          MethodResults>; // method name, method results
    AllResults results_;
