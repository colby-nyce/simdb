...
class CollectionPointBase {
  public:
    CollectionPointBase(uint16_t elem_id, uint16_t clk_id, size_t heartbeat,
        ...
        : argos_record_(elem_id), elem_id_(elem_id), clk_id_(clk_id), heartbeat_(heartbeat),
          dtype_(dtype) {}
    ...
    void sweep(std::vector<char> &swept_data) {
        if (argos_record_.status != ArgosRecord::Status::DONT_READ) {
            swept_data.insert(swept_data.end(), argos_record_.data.begin(),
                              argos_record_.data.end());
        }
...
class CollectionPoint : public CollectionPointBase {
  public:
    ...
    template <typename T>
    typename std::enable_if<type_traits::is_any_pointer<T>::value, void>::type
    ...
    /// Remove this collectable from the black box (do not collect anymore until activate() is
    /// called again).
  ...
  private:
    ...
    template <typename T> void minify_(const T &val) {
        ...
        if (!isAutoCollected()) {
            if (LOG_MINIFICATION)
                std::cout << "[simdb verbose] Manually collected, " << num_bytes << " bytes\n";
        ...
        if constexpr (std::is_trivial<T>::value && std::is_standard_layout<T>::value) {
            if (num_bytes < 16) {
                ...
                if (LOG_MINIFICATION)
                    std::cout << "[simdb verbose] " << num_bytes << "<16, "
...
class ContigIterableCollectionPoint : public CollectionPointBase {
  public:
    ...
    ///    - BOOKENDS: An element was popped off the front and another pushed to the back of the
    ///    container
    ...
    ContigIterableCollectionPoint(uint16_t elem_id, uint16_t clk_id, size_t heartbeat,
                                  const std::string &dtype, size_t capacity)
        : CollectionPointBase(elem_id, clk_id, heartbeat, dtype), curr_snapshot_(capacity),
  ...
  private:
    /// This class is used to compare the current container elements to the previous (last collected
    /// cycle). We do this to determine the most efficient way to write the container to the
    ...
    class IterableSnapshot {
      public:
        ...
        void compareAndMinify(IterableSnapshot &prev, CollectionBuffer &buffer, uint64_t tick,
                              uint16_t elem_id, bool is_auto_collected) {
            uint16_t changed_idx = 0;
            switch (getMinificationAction_(prev, changed_idx, is_auto_collected)) {
            case Action::CARRY:
                if (LOG_MINIFICATION)
                    std::cout << "[simdb verbose] cid " << elem_id << ", tick " << tick
                              << ", CARRY\n";
            ...
            case Action::ARRIVE:
                if (LOG_MINIFICATION)
                    std::cout << "[simdb verbose] cid " << elem_id << ", tick " << tick
                              << ", ARRIVE " << bytes_by_bin_[changed_idx].size() << " bytes\n";
            ...
            case Action::BOOKENDS:
                if (LOG_MINIFICATION)
                    std::cout << "[simdb verbose] cid " << elem_id << ", tick " << tick
                              << ", BOOKENDS, appended " << bytes_by_bin_[changed_idx].size()
                              << " bytes\n";
            ...
            case Action::FULL:
                ...
                for (uint16_t idx = 0; idx < num_elems; ++idx) {
                    if (idx == 0) {
                        ...
                        throw DBException(
                            "All elements in the container must have the same number of bytes");
                    }
      ...
      private:
        Action getMinificationAction_(IterableSnapshot &prev, uint16_t &changed_idx,
            ...
            if (prev_size == curr_size) {
                // If the (ith+1) of the prev container == the ith of the current container, then
                // return BOOKEND
    ...
    template <typename T> void minify_(const T &container) {
        ...
        curr_snapshot_.compareAndMinify(prev_snapshot_, buffer, getTick_(), getElemId(),
                                        isAutoCollected());
...
class SparseIterableCollectionPoint : public CollectionPointBase {
  public:
    SparseIterableCollectionPoint(uint16_t elem_id, uint16_t clk_id, size_t heartbeat,
                                  const std::string &dtype, size_t capacity)
  ...
  private:
    ...
    template <typename T> void minify_(const T &container) {
        ...
        buffer << num_valid;
        if (LOG_MINIFICATION)
            std::cout << "[simdb verbose] num valid: " << num_valid << "\n";
