...
class DatabaseManager {
  public:
    ...
    void EXECUTE(const std::string &sql_cmd, bool in_transaction = true) {
        if (in_transaction) {
            db_conn_->safeTransaction([&]() {
                auto rc = SQLiteReturnCode(sqlite3_exec(db_conn_->getDatabase(), sql_cmd.c_str(),
            ...
            auto rc = SQLiteReturnCode(
                sqlite3_exec(db_conn_->getDatabase(), sql_cmd.c_str(), nullptr, nullptr, nullptr));
            if (rc) {
                throw DBException(sqlite3_errmsg(db_conn_->getDatabase()));
            }
        }
  ...
  private:
    ...
    std::unique_ptr<SqlRecord> findRecord_(const char *table_name, const int db_id,
