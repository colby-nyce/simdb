 ...
 * \class SqlTable
 *
 * \brief Helper class that is used under the hood for db_mgr.INSERT(..., SQL_COLUMNS("ColA",
 * "ColB"), ...)
 */
class SqlColumns {
  public:
    template <typename... Rest>
    SqlColumns(const char *col_name, Rest... rest) : SqlColumns(std::forward<Rest>(rest)...) {
...
class SqlValues {
  public:
    ...
    SqlValues(T val, Rest... rest) : SqlValues(std::forward<Rest>(rest)...) {
...
template <typename T>
inline T queryPropertyValue(const char *table_name, const char *col_name, const int db_id,
                            sqlite3 *db_conn) {
...
inline void SqlRecord::setPropertyBlob(const char *col_name, const void *data,
